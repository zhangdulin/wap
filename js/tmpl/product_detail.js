...
$(function (){
    ...
    template.helper('strToint', function(str) {
       ...
       success:function(result){
          ...
          if(!data.error){
            ...
            $(".minus-wp").click(function (){
               var buynum = $(".buy-num1").val();
               if(buynum >1){
                  $(".buy-num1").val(parseInt(buynum-1));
               }
            });
            //购买数量加
            $(".add-wp").click(function (){
               var buynum = parseInt($(".buy-num1").val());
               if(buynum < data.goods_info.goods_storage){
                  $(".buy-num1").val(parseInt(buynum+1));
                  $('#buynum1').show();
				          $(".buy-num1").css('background-color','#fd0100');
				          $(".buy-num1").css('color','#fff');
               }
            ...
            if(key==''){
              $("input[name='buynum1']").val('0');
            }else{
              $.ajax({
                url:ApiUrl+"/index.php?act=member_cart&op=cart_sum1",
                type : 'post',
                data :{key:key},
                success:function(result){
                    console.log(result)
                    $("input[name='buynum1']").val(result);
                    if(result == 0){
                        $('#buynum1').hide();
                    }
                }
            ...
            $(".add-to-cart").click(function (){
              var key = getcookie('key');//登录标记
               if(key==''){
                  ...
                  $.ajax({
                    url :ApiUrl +"/index.php?act=member_cart&op=check_cart",
                    data: {goods_id:goods_id,key:key},
                    type :"post",
                    success:function(result){
                      if(!result){
            ...
            $(".add-to-cart-all").click(function (){
              var key = getcookie('key');//登录标记
               if(key==''){
                  ...
                  var goods_id_str = $('#gbcc_id').val();
                  console.log(goods_id_str);
                  $.ajax({
                     url:ApiUrl+"/index.php?act=member_cart&op=cart_add_all",
                     data:{key:key,goods_id_str:goods_id_str},
                     type:"post",
                     success:function (result){
                      console.log(result);
                        var rData = $.parseJSON(result);
            ...
            if (data.goods_info.is_virtual == '1') {
                $(".buy-now").click(function() {
                    ...
                    var buynu = parseInt($('.buy-num').val()) || 0;
                    if (buynum < 1) {
                          $.sDialog({
                              skin:"red",
                              content:'购买数量不能为零！',
   ...
   /*         $.ajax({
                type: 'post',
                url: ApiUrl + "/index.php?act=goods&op=comments_list",
                data: {
                    goods_id: goods_id,
                },
                dataType: 'json',
                success: function(result) {
                        var data = result.datas;
                        var html = template.render('comment_list_tmp', data);
                        $("#product_comment").html(html);
                         var stars_url =  SiteUrl + "/data/resource/js/jquery.raty/img/star-on.png";
                         var stars = '<img src="'+stars_url+'">';
                         var num = $('.raty').attr('data-score');
                         var str='';
                         for(var i=0; i<num;i++)
                         {
                            str += stars;
  ...
  function check_Favor() {
    ...
    var goods_id = GetQueryString("goods_id");
    $.ajax({
      url :ApiUrl +"/index.php?act=member_favorites&op=favorites_check",
      data: {goods_id: goods_id,key:key},
      type:"get",
      success:function(res) {
          if (res){
              var favor='已收藏';
			  $('.pd-collect-img').attr('src','../images/gooddetial/collote.png');
          }else {
              var favor='收藏';
          }
          $('.pd-collect').html(favor);
      }
