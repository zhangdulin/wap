	...
	$.ajax({//提交订单信息
		...
		success:function(result){
			...
			$.each(data.store_cart_list,function(k,v){//循环店铺
				...
				i++;
				//htmldata+=
				//<p class="buys-yt-tlt">店铺名称：'+v.store_name+'<span data-store_id="'+k+'" //class="store-cod-supported" style="display:none;">（该店铺不支持选定收货地址的货到付款）</span></p>';-->
						$.each(v.goods_list,function(k1,v1){//循环商品列表
							...
							htmldata+='<div class="clearfix">'
												...
												+'</a>'
												+'<div class="buys1-pdlcnt clearfix">'
												+'<ul class="clearfix">'
													+'<li style="padding-top: 3px;"><a class="buys1-pdlc-name" href="'+WapSiteUrl+'/tmpl/product_detail.html?goods_id='+v1.goods_id+'">'+v1.goods_name+'</a></li>'
													+'<li style=" margin-top:5px; float:right;color:#1c1c1c;">￥'+v1.goods_price+'</li>'
													+'<li style="float:right;color:#8f8f8f;line-height:42px;margin-left:81%;">×'+v1.goods_num+'</li>'                                 +'</ul>'
												+'</div>'
											+'</div>'
										+'</div>';
						});
//循环找到邮费
	    var maxgoodsfreight =0;
			$.each(data.store_cart_list,function(k,v){
				$.each(v.goods_list,function(k1,v1){
					$.each(v1.goods_freight,function(k2,v2){
			          if (maxgoodsfreight < v2) {
			          	maxgoodsfreight = v2;
			          };
					});
				});
						...
						//满减显示问题
                        var freight_message ="";
                        $.each(data.store_cart_list,function(q,w){
                        	if(w.freight_message){
                            freight_message =parseInt(w.freight_message.replace(/[^0-9]/ig,""));
                            }else{
                            	return true;
                            }
                        });
                        if(freight_message &&freight_message !=""){
                        htmldata+= '<p style="float:left; margin-left:8px; line-height:42px;color:#1c1c1c;">发货方式</p>'+'<div class="shop-total"><p style="float:right;margin-left:10px;">快递：￥<span class="maxgoodsfreight">0</span></p>';
                        }else{
						htmldata+= '<p style="float:left; margin-left:8px; line-height:42px;color:#1c1c1c;">发货方式</p>'+'<div class="shop-total"><p style="float:right;margin-left:10px;">快递：￥<span class="maxgoodsfreight">'+maxgoodsfreight+'</span></p>';
					    }
						// htmldata+= '<p style="float:left; margin-left:8px; line-height:42px;color:#1c1c1c;">发货方式</p>'+'<div class="shop-total"><p style="float:right;margin-left:10px;">快递：￥<span id="store'+k+'"></span></p>';
						
	                    htmldata+='</div>';
                        htmldata+='</li>';
						if(v.store_mansong_rule_list != null){
							htmldata+= '<div style=" margin-left: -56px;float: left;margin-top: 10px;">满即送-'+v.store_mansong_rule_list.desc+':-'+v.store_mansong_rule_list.discount+'</div>';
						}
                       ...
                       // }
						...
						// 	htmldata+='<p class="clr-c07 clearfix" style="margin-top: 25px;margin-left:8px;border-top: 1px solid #e8e8e8;border-bottom:1px solid #e8e8e8;height: 40px;line-height: 40px;">订单总计<span id="st'+k+'" store_price="'+sp_total+'" class="store_total">￥'+p2f(sp_total)+'</span></p>';

						//原始总的商品价
				      $("input[name=total_priceold]").val(v.store_goods_total);
				      //新加优惠券
						var discountprice =GetQueryString('discountprice');
						if(discountprice != null && discountprice != ""){

							var sp_total =pf(v.store_goods_total);     
							htmldata+='<p class="clr-c07 clearfix" style="margin-top: 25px;margin-left:8px;border-top: 1px solid #e8e8e8;border-bottom:1px solid #e8e8e8;height: 40px;line-height: 40px;" onclick="choiceyouhui()">优惠方式<span class="store_totaljian"></span><span style="color:#fe0000;padding-right:6px;float: right;letter-spacing: 1px;">满'+discountprice+'减'+discountprice+'</span></span></p>'
							+'<p class="clr-c07 clearfix" style="margin-left:8px;border-bottom:1px solid #e8e8e8;height: 40px;line-height: 40px;">订单总计<span id="st'+k+'" store_price="'+sp_total+'" class="store_total">￥'+p2f(sp_total)+'</span></p>';
							
						}else{
							var sp_total = pf(v.store_goods_total);
							htmldata+='<p class="clr-c07 clearfix" style="margin-top: 25px;margin-left:8px;border-top: 1px solid #e8e8e8;border-bottom:1px solid #e8e8e8;height: 40px;line-height: 40px;" onclick="choiceyouhui()">优惠方式<span class="store_totaljian"></span></span></p>'
							   +'<p class="clr-c07 clearfix" style="margin-left:8px;border-bottom:1px solid #e8e8e8;height: 40px;line-height: 40px;">订单总计<span id="st'+k+'" store_price="'+sp_total+'" class="store_total">￥'+p2f(sp_total)+'</span></p>';
						}
            ...
            $('#new-address-button').click(function() {
				...
				$('#new-address-imgs').attr('src','../../images/gooddetial/jiantou2.png');
				$('#new-address-imgs').css('height','12px');
				$('#new-address-imgs').css('margin','25px 16px 0 0;');
				$('.buys1-pdlcnt').css('display','none');
				$('.buys-ytable').css('display','none');
				$('.buys-dis-mei').css('display','none');
            });
...
//循环找到邮费
        var discountprice = 0;
	        discountprice =GetQueryString('discountprice');
	    var maxgoodsfreight =0;
			$.each(data.store_cart_list,function(k,v){
				$.each(v.goods_list,function(k1,v1){
					$.each(v1.goods_freight,function(k2,v2){
			...
			$.ajax({//保存地址
				...
				success:function(result){
					if(result.datas.state == 'success'){
						...
						//满减显示问题
                        var currenttotal =parseInt($('input[name=total_priceold]').val());
                        var freight_message ="";
                        $.each(data.store_cart_list,function(q,w){
                        	if(w.freight_message){
                            freight_message =w.freight_message.replace(/[^0-9]/ig,"");
                            }else{
                            	return true;
                            }
                        });
                        if(currenttotal > freight_message &&freight_message !=""){
                         var total_price = currenttotal -discountprice;
                        }else{
						var total_price = currenttotal +parseInt(maxgoodsfreight)  -discountprice;
					    }
//新该总价
					  //var total_pricenew =parseInt(currenttotal) +parseInt(maxgoodsfreight);
						$('#total_price').html(total_price);
						$('.store_total').html(total_price);
						//$('input[name=total_price]').val(total_price);
        ...
        $.ajax({
        	...
        	success:function(result){
        		...
        		if(result.datas.pay_sn.pay_sn != ''){
        			var discountprice =GetQueryString('discountprice');
        			var maxgoodsfreight = $('.maxgoodsfreight').html();
       				//检测和传paysn，pay_amount
       			    var pay_sn =result.datas.pay_sn;//pay_sn
       				var currenttotal =parseInt($('input[name=total_priceold]').val());
                    //var pay_amount = $("#total_price").html(); 
                    var pay_amount =parseInt(currenttotal) +parseInt(maxgoodsfreight);
				    if(currenttotal > 168){
				    	pay_amount = currenttotal;
				    	$("#total_price").html(currenttotal);
				        }
				    if (discountprice !="" && discountprice !=null) {
				    	var currenttotalnew =pay_amount -discountprice;
				    	$("#total_price").html(currenttotalnew);
				    };
                    ...
                    if(ua.match(/MicroMessenger/i) == 'micromessenger'){
                    	// 优惠券的使用
	        			if (discountprice != ""){
	        			  	window.location.href = ApiUrl+'/api/payment/wxpay_v3/pay/jsapi.php?pay_sn='+pay_sn+'&money='+discountprice+'&amount='+pay_amount;
	        			
        		       }else{
        		      		window.location.href =ApiUrl+'/api/payment/wxpay_v3/pay/jsapi.php?pay_sn='+pay_sn+'&amount='+pay_amount;
        		       }
        		   }else{
        		   	if (discountprice !="") {
                    var pay_amount = pay_amount -discountprice;
        		   	};
        		   	var wxpayurl = "wxpay://"+pay_amount+"/"+pay_sn;
                                 //alert(wxpayurl);
                       window.location.href = wxpayurl;
