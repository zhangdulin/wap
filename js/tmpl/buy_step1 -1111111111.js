				...
				//<p class="buys-yt-tlt">店铺名称：'+v.store_name+'<span data-store_id="'+k+'" //class="store-cod-supported" style="display:none;">（该店铺不支持选定收货地址的货到付款）</span></p>';-->
				      ...
				      //新加优惠券
						...
						if(discountprice != null){
							...
							// var maxfreight = '';
						 //         maxfreight =0;
						 //    goodsLists =v.goods_list;
						 //   $.each(goodsLists,function(x,p){
						 //    	goodsfreight =p.goods_freight;
							// 	   for(var i=0;i<goodsfreight.length;i++){
							// 	   	if(maxfreight < goodsfreight[i]){
							// 	   		maxfreight = goodsfreight[i];
						 //   			}
						 //   		}
						
						 //    });
							//var sp_total =newPrice; 
			...
			$.ajax({//保存地址
				...
				success:function(result){
					if(result.datas.state == 'success'){
                        ...
                        if(currenttotal >freight_message){
                         var total_price = pf($('input[name=total_price]').val()) + sp_s_total -discountprice;
                        }else{
						var total_price = pf($('input[name=total_price]').val()) + sp_s_total -discountprice;
					    }
						$('#total_price').html(p2f(total_price));
        ...
        $.ajax({
        	...
        	success:function(result){
        		...
        		if(result.datas.pay_sn.pay_sn != ''){
                     ...
                     var pay_amount = $("#total_price").html(); //pay_amount
                    if (!readytopay) {
                        //callpay("111","111","111");
        			...
        			var discountprice =GetQueryString('discountprice');
        			if (discountprice !="") {
        			  window.location.href =ApiUrl+'/api/payment/wxpay_v3/pay/jsapi.php?pay_sn='+pay_sn+'&money='+discountprice;
        			
        		       }else{
        		      window.location.href =ApiUrl+'/api/payment/wxpay_v3/pay/jsapi.php?pay_sn='+pay_sn;
        		       }
        		}
        		return false;
        	}
